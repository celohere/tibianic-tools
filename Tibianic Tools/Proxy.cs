...
namespace TibianicTools
{
    ...
    class Proxy
    {
        ...
        internal static List<byte[]> RecordedPackets = new List<byte[]>();
        ...
        private static void HandleServerComm()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        if (Recording && connection > 5)
                        {
                            ...
                            byte[] Packet = new byte[1], packageLength = new byte[1];
                            Packet = new byte[packetToSend.Length + 8]; // 4 bytes needed for delay, 4 bytes needed for package length
                            packageLength = BitConverter.GetBytes(bytesRead);
                            TimeRecorded = stopwatchRecordingStarted.ElapsedMilliseconds;
                            byte[] Delay = BitConverter.GetBytes((int)TimeRecorded);
                            Array.Copy(Delay, Packet, 4);
                            Array.Copy(packageLength, 0, Packet, 4, 4);
                            Array.Copy(packetToSend, 0, Packet, 8, packetToSend.Length);
                ...
                if (Memory.ReadByte(Addresses.Client.Connection) == 8 && Recording)
                {
                    if (!listenThread.IsAlive)
                    {
                        listenerLocal = new TcpListener(IPAddress.Parse("127.0.0.1"), ServerPort);
        ...
        internal static void ClearTibiaCamBuffer()
        {
            ...
            RecordedPackets.Add(buffer);
            RecordedPackets.Add(new byte[] { 0, 0, 0, 0 }); // Later to be filled in with total running length (last packet's sleep) 4 BYTES NEEDED
        }
